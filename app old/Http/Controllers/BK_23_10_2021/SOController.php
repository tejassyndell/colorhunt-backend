...
// 		}
		...
		//$vnddataTotal = DB::select("select count(*) as Total from (select * from (SELECT GetTotalSOOrderPieces(son.Id) as TotalSoPieces, son.Id, p.Name, OutwardSoList(son.Id) as OWID, SalesReturnArticle(GROUP_CONCAT(DISTINCT CONCAT(a.Id) ORDER BY son.Id SEPARATOR ',')) as SalesRetrunAssign, GROUP_CONCAT(DISTINCT CONCAT(a.ArticleNumber) ORDER BY son.Id SEPARATOR ',') as ArticleNumber, son.SoDate,DATE_FORMAT(son.SoDate, \"%d/%m/%Y\") as cdate, son.Destination, son.Transporter, son.UserId, concat(FirstCharacterConcat(u.Name), son.SoNumber, '/',fn.StartYear,'-',fn.EndYear) as SoNumber FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId group by s.SoNumberId) as ddd where ddd.OWID=0 order by ddd.Id desc) as d ".$whereuser);
		//$vnddataTotal = DB::select('SELECT son.Id, own.Id FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId left join outwardnumber own on own.SoId= son.Id where son.id in (son.Id) and own.Id is NULL '.$whereuser.' group by s.SoNumberId');
		...
		//$vnddata = DB::select("select d.* from (select * from (SELECT GetTotalSOOrderPieces(son.Id) as TotalSoPieces, son.Id, s.Status, p.Name, OutwardSoList(son.Id) as OWID, SalesReturnArticle(GROUP_CONCAT(DISTINCT CONCAT(a.Id) ORDER BY son.Id SEPARATOR ',')) as SalesRetrunAssign, GROUP_CONCAT(DISTINCT CONCAT(a.ArticleNumber) ORDER BY son.Id SEPARATOR ',') as ArticleNumber, son.SoDate,DATE_FORMAT(son.SoDate, \"%d/%m/%Y\") as cdate, son.Destination, son.Transporter, son.UserId, concat(FirstCharacterConcat(u.Name), son.SoNumber, '/',fn.StartYear,'-',fn.EndYear) as SoNumber FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId group by s.SoNumberId) as ddd where ddd.OWID=0 and ddd.Status!=1 order by ddd.Id desc) as d ".$wherecustom." ".$searchstring." ".$order." limit ".$data['dataTablesParameters']["start"].",".$length);
		$vnddata = DB::select("select d.* from (SELECT GetTotalSOOrderPieces(son.Id) as TotalSoPieces, son.Id, s.Status, p.Name, (CASE WHEN own.Id IS NULL THEN 0 ELSE 1 END) as OWID, SalesReturnArticle(GROUP_CONCAT(DISTINCT CONCAT(a.Id) ORDER BY son.Id SEPARATOR ',')) as SalesRetrunAssign, GROUP_CONCAT(DISTINCT CONCAT(a.ArticleNumber) ORDER BY son.Id SEPARATOR ',') as ArticleNumber, son.SoDate,DATE_FORMAT(son.SoDate, '%d/%m/%Y') as cdate, son.Destination, son.Transporter, son.UserId, concat(FirstCharacterConcat(u.Name), son.SoNumber, '/',fn.StartYear,'-',fn.EndYear) as SoNumber FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId left join outwardnumber own on own.SoId= son.Id where own.Id is NULL  and s.Status!=1 ".$whereuser." group by s.SoNumberId) as d  ".$searchstring." ".$order." limit ".$data['dataTablesParameters']["start"].",".$length);
		
	
	    //$vnddata = DB::select("select d.* from (SELECT GetTotalSOOrderPieces(son.Id) as TotalSoPieces, son.Id, s.Status, p.Name, (CASE WHEN own.Id IS NULL THEN 0 ELSE 1 END) as OWID, SalesReturnArticle(GROUP_CONCAT(DISTINCT CONCAT(a.Id) ORDER BY son.Id SEPARATOR ',')) as SalesRetrunAssign, GROUP_CONCAT(DISTINCT CONCAT(a.ArticleNumber) ORDER BY son.Id SEPARATOR ',') as ArticleNumber, son.SoDate,DATE_FORMAT(son.SoDate, '%d/%m/%Y') as cdate, son.Destination, son.Transporter, son.UserId, concat(FirstCharacterConcat(u.Name), son.SoNumber, '/',fn.StartYear,'-',fn.EndYear) as SoNumber FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId left join outwardnumber own on own.SoId= son.Id where own.Id is NULL or own.DeleteOutward=1 and s.Status!=1 ".$whereuser."  group by s.SoNumberId) as d ".$searchstring." ".$order." limit ".$data['dataTablesParameters']["start"].",".$length);
	
	
		//return "select d.* from (SELECT GetTotalSOOrderPieces(son.Id) as TotalSoPieces, son.Id, s.Status, p.Name, (CASE WHEN own.Id IS NULL THEN 0 ELSE 1 END) as OWID, SalesReturnArticle(GROUP_CONCAT(DISTINCT CONCAT(a.Id) ORDER BY son.Id SEPARATOR ',')) as SalesRetrunAssign, GROUP_CONCAT(DISTINCT CONCAT(a.ArticleNumber) ORDER BY son.Id SEPARATOR ',') as ArticleNumber, son.SoDate,DATE_FORMAT(son.SoDate, '%d/%m/%Y') as cdate, son.Destination, son.Transporter, son.UserId, concat(FirstCharacterConcat(u.Name), son.SoNumber, '/',fn.StartYear,'-',fn.EndYear) as SoNumber FROM `so` s inner join article a on a.Id=s.ArticleId left join sonumber son on s.SoNumberId=son.Id inner join party p on p.Id=son.PartyId inner join financialyear fn on fn.Id=son.FinancialYearId inner join users u on u.Id=son.UserId left join outwardnumber own on own.SoId= son.Id where own.Id is NULL and s.Status!=1  group by s.SoNumberId) as d ".$wherecustom." ".$searchstring." ".$order." limit ".$data['dataTablesParameters']["start"].",".$length;
...
// 		   ->rightJoin('mixnopacks', 'mixnopacks.ArticleId', '=', 'article.Id') 
// 		   -> get();
...
// 		  //  Latest Commented 
//           // return DB::select("SELECT mxp.NoPacks, a.ArticleNumber, ar.ArticleRate, a.ArticleOpenFlag FROM `mixnopacks` mxp inner join article a on a.Id=mxp.ArticleId left join articlerate ar on ar.ArticleId=a.Id where mxp.ArticleId ='".$ArticleId."'");
// 		 $datasso =  DB::select("SELECT a.ArticleNumber, ar.ArticleRate, a.ArticleOpenFlag, mxp.NoPacks FROM `mixnopacks` mxp inner join article a on a.Id=mxp.ArticleId left join articlerate ar on ar.ArticleId=a.Id where mxp.ArticleId ='".$ArticleId."'");
// // 		foreach($datasso as $so){
// // 				$so->NoPacks = $TotalRemaining;
// // 			}
// 		return $datasso;    
