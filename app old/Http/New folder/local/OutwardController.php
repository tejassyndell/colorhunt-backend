...
echo $inwardsale."\n";
	...
	public function RemainingPostOutwardSO(Request $request)
	{
		...
		if($search['value'] != null && strlen($search['value']) > 2){
			$searchstring = " WHERE d.Name like '%".$search['value']."%' Or d.ArticleNumber like '%".$search['value']."%' OR cast(d.SoDate as char) like '%".$search['value']."%'";
	...
	public function GetSOData($OWNO, $Id){
		...
		$allarticles = DB::select("select * from (select s.ArticleId, a.ArticleNumber, OutwardNoPacksCheck('".$Id."', s.ArticleId) as OutwardNoPacksCheck from so s inner join article a on a.Id=s.ArticleId  where s.SoNumberId='".$Id."') as d where OutwardNoPacksCheck=0");
