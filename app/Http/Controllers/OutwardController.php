    ...
    public function UpdateOutward(Request $request)
    {
        ...
        if ($result[0]->ArticleOpenFlag == 0) {
            ...
            if ($existingRecord) {
                ...
                if (!empty($getresult)) {
                    $GetNoPacks = (int)$getresult[0]->SalesNoPacks; // Cast to integer
                    $dataupdate = $GetNoPacks + (int)$data['NoPacksNew']; // Cast to integer
                    DB::table('artstockstatus')->where(['outletId' => $data['PartyId']])->where(['ArticleId' => $articleId])->update(['SalesNoPacks' => $dataupdate, 'TotalPieces' => $dataupdate]);
                } else {
                    $dataupdate = (int)$data['NoPacks'] + (int)$data['NoPacksNew']; // Cast to integer
                    DB::table('artstockstatus')->where(['outletId' => $data['PartyId']])->where(['ArticleId' => $articleId])->update(['SalesNoPacks' => $dataupdate, 'TotalPieces' => $dataupdate]);
                }
            }
        }
