    ...
    public function GetOutletStocks($PartyId)
    {

        {
            ...
            foreach ($articles as $key => $article) {
                ...
                $allRecords = DB::select("SELECT dd.NoPacks, dd.type FROM (
                    SELECT osr.NoPacks, 2 AS type, osrn.CreatedDate AS SortDate FROM outletsalesreturn osr INNER JOIN outletsalesreturnnumber osrn ON osr.SalesReturnNumber = osrn.Id WHERE osr.ArticleId = ? AND osr.OutletPartyId = ?
                    UNION
                    SELECT o.NoPacks, 1 AS type, onum.CreatedDate AS SortDate FROM outlet o INNER JOIN outletnumber onum ON o.OutletNumberId = onum.Id WHERE o.ArticleId = ? AND onum.PartyId = ?
                    UNION
                    SELECT o.NoPacks, 0 AS type, to1.ReceivedDate AS SortDate FROM outward o INNER JOIN transportoutlet to1 ON o.OutwardNumberId = to1.OutwardNumberId INNER JOIN outwardnumber on1 ON o.OutwardNumberId = on1.Id WHERE o.ArticleId = ? AND to1.TransportStatus = 1 AND o.PartyId = ?
                    UNION
                    SELECT sr.NoPacks, 3 AS type, srn.CreatedDate AS SortDate FROM outward o INNER JOIN salesreturn sr ON sr.OutwardId = o.Id
                    INNER JOIN salesreturnnumber srn ON srn.Id = sr.SalesReturnNumber WHERE o.PartyId = ? AND o.ArticleId = ?
                ) AS dd WHERE SortDate > ? ORDER BY dd.SortDate ASC", [
                    $articleId, $partyId,$articleId, $partyId,
                    $articleId,$partyId, $partyId,$articleId,$dateThreshold
                ]);
